					...
					<div class="card-group">
						...
						<c:if test="${not empty meals}">
							...
							<c:forEach var="meal" items="${meals}">
								...
								<div class="card">
									<div class="card-body">
										<img src="/images/${meal.mealphoto}" class="card-img-top" style="height: 20rem;width: 23rem;object-fit: contain;">
										 	<%-- <div class="form-group">
					                            <label>Logged-in User:</label>
					                            <p>${pageContext.request.userPrincipal.name}</p>
	                        				</div> --%>
										...
										<small class="text-muted"> 
											<%-- <c:if test="${empty meals}"> --%>
												<sec:authorize access="hasRole('Member')">
													<a onclick="return orderConf(event)" id="order-link" href="order?id=${meal.meal_id}&userId=${pageContext.request.userPrincipal.name}" >
